...
========================
This guide will walk you through setting up a local development environment for contributing to ``ampworks``. It covers recommended practices, tools, and commands for developers to efficiently build, test, and contribute to the project.
...
3. Install ``ampworks`` in editable mode
    Once you have your virtual environment activated and the files locally available, install ``ampworks`` in editable mode, including the necessary development tools and dependencies, like so::
...
6. Building documentation
    ...
    You can see the local documentation build in your browser by opening the ``index.html`` file from the ``docs/build/`` folder.

Now that you're all setup with a development version of ``ampworks`` and have tested the codebase using the ``nox`` integration, be sure to follow the :doc:`version_control` workflow as you contribute. Happy coding!
